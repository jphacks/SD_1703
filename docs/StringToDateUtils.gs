// Optional<String> year, Optional<String> month, Optional<String> day, String hour, String minute,
function newDate(day, hour, minute) {
  const now = Date.now();
  return new Date(now.getFullYear(), month.getFullYear(), day, hour, minute);
}

function newDate(day, hour) {
  const now = Date.now();
  return new Date(now.getFullYear(), month.getFullYear(), day, hour);
}
